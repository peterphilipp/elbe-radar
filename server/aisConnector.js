...
class AISConnector {
  constructor(onUpdate) {
    ...
    this.currentBox = { n: 53.620, s: 53.490, w: 9.100, e: 10.050 };
  }
  ...
  // Neue BoundingBox vom Browser – resende Subscription
  updateBox(box) {
    this.currentBox = box;
  ...
  _subscribe() {
    ...
    console.log(`[AIS] Subscription: N${b.n} S${b.s} W${b.w} E${b.e}`);
  }
  ...
  _connect() {
    ...
    this.ws.on('message', data => {
      try { this._handleMsg(JSON.parse(typeof data === 'string' ? data : data.toString('utf8'))); }
      catch (e) { console.error('[AIS] Parse-Fehler:', e.message); }
    });
  ...
  _handleMsg(d) {
    ...
    this._upsert({ mmsi, lat, lon,
      name: (meta.ShipName || '').trim() || ex.name || 'Unbekannt',
      type: ex.type || shipType(null, (meta.ShipName || '')) || 'Cargo',
      dest: ex.dest || '', cs: ex.cs || '', len: ex.len || 0, wid: ex.wid || 0, drg: ex.drg || 0,
  ...
  _upsert(p) {
    ...
    if (eta) { merged.eta = eta; checkAlert(merged, eta); }
    this._cleanup();
  ...
  _cleanup() {
    ...
    for (const [mmsi, s] of this.ships) if ((s.seen || 0) < cutoff) this.ships.delete(mmsi);
  }
  ...
  _startDemo() {
    ...
    setInterval(() => {
      for (const s of this.ships.values()) {
        ...
        s.seen = Date.now();
        const eta = calcETA(s); if (eta) { s.eta = eta; checkAlert(s, eta); }
      }
      this.onUpdate(this.ships);
    }, 3000);
    console.log('[AIS] Demo-Modus');
  }
}
